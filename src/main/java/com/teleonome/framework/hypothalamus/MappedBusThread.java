			...
			while(keepRunning){
				...
				for(Enumeration en=hypothalamus.microControllerPointerMicroControllerIndex.keys();en.hasMoreElements();){
					...
					try {
						...
						if(o !=null && o instanceof Boolean && ((Boolean)o))hasUserCommands=true;
					...
					if(hasUserCommands) {
						logger.debug("starting the usar commands" );

						try {

							output = aMicroController.getWriter();
							logger.debug("requesting " + TeleonomeConstants.USER_COMMAND);
							output.write(TeleonomeConstants.USER_COMMAND,0,TeleonomeConstants.USER_COMMAND.length());
							...
							if(ready){
								...
								do {
									...
									logger.info(aMicroController.getName()  + " was sent=" + TeleonomeConstants.USER_COMMAND + " received=" + inputLine);
									try {
										Thread.sleep(500);
									} catch (InterruptedException e) {
										// TODO Auto-generated catch block
									...
									if(inputLine.startsWith("Ok")) {
										...
										if(inputLine.equals(TeleonomeConstants.COMMAND_REBOOT)){
											...
											//
											// if we are here is because is a device specific user command
											// these user commands will come from physical buttons attached
											// to the micro controller therefore there is no code
											// 
											// UserCommand#MutationToExecute#DataTarget#Data
											//
											// for example: 
											//    mutation Name= MusicPlayerAction
											//    dataTarget=@On Load:Update Action Command:Update Action Command
											//    data="SetVolume#45";
											//
											tokens = inputLine.split("#");
											String mutationName=tokens[1];
											String dataTarget = tokens[2];
											...
											if(data!=null && !data.equals("")) {
												...
												try {
													dataPayloadJSONObject.put("Mutation Name",mutationName);
...
//													updatesArray.put(updateJSONObject);
													...
													updateJSONObject.put(TeleonomeConstants.MUTATION_PAYLOAD_UPDATE_TARGET,dataTarget);
													updateJSONObject.put(TeleonomeConstants.MUTATION_PAYLOAD_VALUE ,data);
													updatesArray.put(updateJSONObject);
												} catch (JSONException e) {
													// TODO Auto-generated catch block
													logger.warn(Utils.getStringException(e));
												}
												logger.debug("line 274 dataPayloadJSONObject=" + dataPayloadJSONObject);
											}
											...
											clientIp="127.0.0.1";
											commandCode="";
											JSONObject aCommandRequestJSONObject = hypothalamus.aDBManager.requestCommandToExecute(mutationName,commandCode,TeleonomeConstants.TELEONOME_SECURITY_CODE, dataPayloadJSONObject.toString(), clientIp, restarRequired);	
											logger.debug("line 276 user command as request= " + aCommandRequestJSONObject);
											
											if(aCommandRequestJSONObject!=null){
												
												if(mutationName!=null && !mutationName.equals("") ){
													logger.debug("line 288  to execute mutationName=" + mutationName);
													...
													if(dataPayloadJSONObject!=null){
														...
														mutationJSONObject = hypothalamus.aDenomeManager.readImmediateMutation(command);
													}
													...
													try {
														...
														logger.info("line 322 mutationJSONObject " + mutationJSONObject);
													...
													logger.debug("about to mark commandrequest as cmpleted, " + aCommandRequestJSONObject.getInt("id"));
													JSONObject commandResponseJSONObject = hypothalamus.aDenomeManager.markCommandCompleted(aCommandRequestJSONObject.getInt("id"));
													hypothalamus.publishToHeart(TeleonomeConstants.HEART_TOPIC_UPDATE_FORM_RESPONSE, commandResponseJSONObject.toString());
													
													//
													// End oF Executing the Command
													//
				...
				//
				// Third get AsyncData
				// Single-mode microcontrollers are polled once per Async Cycle.
				// Continuous-mode microcontrollers are polled every iteration of the
				// inner while loop until PulseThread sets keepRunning=false.
				...
				logger.debug("Executing AsyncData");
				long asyncRequestDelayMillis=0;
				Set<String> asyncSingleDone = new HashSet<>();
				while(keepRunning) {
					for(Enumeration en=hypothalamus.microControllerPointerMicroControllerIndex.keys();en.hasMoreElements();){
						microControllerPointer = (String)en.nextElement();
						aMicroController = (MicroController)hypothalamus.microControllerPointerMicroControllerIndex.get(microControllerPointer);
						...
						if(!aMicroController.isEnableAsyncUpdate()) continue;
						if(!aMicroController.isAsyncContinuous() && asyncSingleDone.contains(microControllerPointer)) continue;
						...
						do {
							try {
								...
								if(inputLine.startsWith("Ok") ||
									...
									keepGoing=false;
									logger.info("set keepGoing=false keepRunning="+ keepRunning);
								}else {
									counter++;
									if(counter>maxCounter) {
										keepGoing=false;
						...
						if(inputLine.startsWith(TeleonomeConstants.HEART_TOPIC_ASYNC_CYCLE_UPDATE)){
							...
							if(sensorRequestQueuePositionDeneWordIndex!=null && sensorDataTokens!=null && sensorDataTokens.length>0) {
								for (Map.Entry<JSONObject, Integer> entry2 : sensorRequestQueuePositionDeneWordIndex) {
									...
									logger.debug("processing sensor token:" + adjustedIndex + " resulted in " + sensorValueString);
							...
							logger.debug("about to send async update to heart " + dataToPublishJSONObject.toString());
						...
						if(!aMicroController.isAsyncContinuous()) {
							asyncSingleDone.add(microControllerPointer);
						}
			...
			for( int i=0;i<teleonomeRememberedDeneChainsArrayList.size();i++) {
				...
				for(int l=0;l<denes.length();l++) {
					...
					for(int j=0;j<deneWords.length();j++) {
						...
						units = (String) getDeneWordByIdentity(currentPulse, includedRememberedIdentity, TeleonomeConstants.DENEWORD_UNIT_ATTRIBUTE);
						logger.debug("about to unwrap " + includedRememberedIdentity.toString() + " with value:" + value  + " and valueType=" + valueType + " units=" + units);
